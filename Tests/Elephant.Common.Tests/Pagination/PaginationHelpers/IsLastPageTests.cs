...
namespace Elephant.Common.Tests.Pagination.PaginationHelpers
{
	...
	public class IsLastPageTests
	{
		...
		[Theory]
		[SpeedVeryFast, UnitTest]
		[InlineData(100, 0, 10)] // First page of 10 pages.
		[InlineData(100, 10, 10)] // Second page of 10 pages.
		[InlineData(100, 50, 10)] // Middle page.
		[InlineData(100, 80, 10)] // Second to last page.
		[InlineData(1000, 0, 100)] // Large dataset, first page.
		[InlineData(1000, 500, 100)] // Large dataset, middle page.
		[InlineData(50, 0, 25)] // Two pages, first page.
		[InlineData(2, 0, 1)] // Two items, first page.
		[InlineData(15, 7, 1)] // Page size of 1, middle page.
		[InlineData(200, 99, 100)] // Just before boundary.
		public void IsLastPageNumber_NotLastPage(int sourceCount, int offset, int limit)
		{
			Assert.False(PaginationHelper.IsLastPage(sourceCount, offset, limit));
		}
		...
		{
			Assert.True(PaginationHelper.IsLastPage(sourceCount, offset, limit));
		}
		...
		[InlineData(false, 10000, 5000, 100)] // Middle page.
		[InlineData(false, 10000, 9800, 100)] // Second to last page.
